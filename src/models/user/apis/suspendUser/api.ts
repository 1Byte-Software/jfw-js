...
import { ISuspendUserOptions } from './types';
 ...
 *
 * @param options - The options for suspend a user.
...
export const suspendUserAPI = async (
    options: ISuspendUserOptions,
    config?: AxiosRequestConfig,
) => {
    const { pathParameters, headerParameters } = options;
    const url = generatePath(USER_PATH.SUSPEND_USER, pathParameters);
